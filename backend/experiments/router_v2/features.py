"""Deterministic query features → difficulty score.

Pure stdlib + regex (NO model inference) so it is reproducible and costs nothing.
`difficulty` is computed FROM THE QUERY TEXT — it is not a fabricated fact about any
model. Weights/knees come from config.yaml (policy knobs), not hard-coded magic.
"""
...
CHARS_PER_TOKEN = 4  # matches the live engine's estimate
...
class QueryFeatures:
    ...
    is_imperative: bool
    ambiguity: float          # 0..1
    difficulty: float = 0.0   # filled by difficulty_score()
...
def extract_features(prompt: str, *, intent_matched: bool = True) -> QueryFeatures:
    ...
    # ambiguity: vague/under-specified pushes difficulty up. A short prompt alone is NOT
    # ambiguous (many easy questions are short) — only penalize shortness mildly, and weight
    # the real signal (no intent matched / dangling references) more.
    amb = 0.0
    if wc < 4:
        amb += 0.15
...
def difficulty_score(f: QueryFeatures, cfg: Dict) -> float:
    """Weighted, clamped 0..1 blend of normalized query signals (weights from config)."""
    w = cfg.get("weights", {})
