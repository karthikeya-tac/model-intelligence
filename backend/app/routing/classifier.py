...
DEFAULT path: MiniLM sentence embeddings vs the routes.yaml utterances, cosine→confidence via a
temperature softmax (calibrated, not hard-coded), returning a ranked top-k of intents so Level-2
can fuse multiple capabilities. Falls back LOUDLY to keyword matching if sentence-transformers /
the model is unavailable, so the service still runs and stays deterministic.
"""
...
log = logging.getLogger("niha.classifier")
...
class IntentClassifier:
    ...
    def _build_semantic(self, routes_path: Path, model_name: str) -> None:
        try:
            ...
            log.warning("niha: embeddings unavailable (%s) — using KEYWORD classification.", e)
            return
        try:
            ...
            log.info("niha: semantic classifier ready (%d routes, %s).", len(self._mat), model_name)
        except Exception as e:
            log.warning("niha: semantic build failed (%s) — using KEYWORD classification.", e)
            self._enc = None
    ...
    def classify(self, prompt: str) -> Dict:
        if self._enc is not None:
            ...
            if ranked and ranked[0][1] >= self.sim_floor:
                return self._finalize(ranked, source="semantic")
        kw = self._keyword_rank(prompt)
        if kw:
            return {"intent_id": kw[0][0], "intents": kw[:self.top_k], "confidence": kw[0][1],
                    "margin": (kw[0][1] - (kw[1][1] if len(kw) > 1 else 0.0)), "source": "keyword"}
    ...
    def _finalize(self, ranked: List[Tuple[str, float]], *, source: str) -> Dict:
        np = self._np
        top_cos = ranked[0][1]
        ...
        return {"intent_id": ranked[0][0], "intents": intents,
                "confidence": round(float(probs[0]), 4), "margin": round(top_cos - second_cos, 4),
                "source": source}
